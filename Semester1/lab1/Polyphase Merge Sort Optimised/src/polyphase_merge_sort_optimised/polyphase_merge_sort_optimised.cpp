...
void distribute_nums(fstream &init_file, fstream files[], vector<int> const &distribution, const int MAX_NUMS, const int NUM_OF_BAD_RUNS) {
...
void pull_number(fstream files[], vector<int> &runs, bool available[], bool &file_processed, int to_read, int prev_nums[]) {
	int num;

	if (available[to_read]) {
		if (files[to_read].peek() != EOF) {
			files[to_read].read(reinterpret_cast<char*>(&num), sizeof(int));
			if (num < prev_nums[to_read]) available[to_read] = false;
			else {
				runs[to_read] = num;
				prev_nums[to_read] = num;
			}
		}
		else {
...
void merge_files(fstream files[], const int NUM_OF_FILES, const int MAX_NUMS) {
	vector<int> runs(NUM_OF_FILES);
	int num_of_empty, to_add;
	int* prev_nums = new int[NUM_OF_FILES];
	...
	while (files[0].peek() != EOF || files[1].peek() != EOF) {
		...
		while (true) {
			if (to_add == -1) {
				for (int i = 0; i < NUM_OF_FILES; i++) {
					if (available[i]) pull_number(files, runs, available, file_processed, i, prev_nums);
				}
			}
			next_step = true;
			for (int i = 0; i < NUM_OF_FILES; i++) {
			...
			if (to_add != -1) {
				if (available[to_add]) pull_number(files, runs, available, file_processed, to_add, prev_nums);
			}
...
int polyphase_merge_sort(int max_nums, int num_of_files, fstream &init_file, bool demo = true) {
	...
	distribute_nums(init_file, files, distribution, max_nums, num_of_bad_runs);
	merge_files(files, num_of_files, max_nums);
...
void benchmark() {
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			for (int k = 0; k < 2; k++) {
				int time = polyphase_merge_sort(ram_limit[k], num_of_files[i], init_file, false);
...
int main()
{
	int max_nums = 10000, num_of_files = 5;

	fstream init_file("initial_file.bin", ios::binary | ios::in | ios::out | ios::trunc);
	if (!(init_file.is_open())) std::cout << "Cannot open bin file\n";

	generate_bin(init_file, 1000000);
	polyphase_merge_sort(max_nums, num_of_files, init_file);
	//benchmark();
	return 0;
}